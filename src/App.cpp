...
App::App(int argc, char** argv) {
    ...
    // Create the window
    _window = glfwCreateWindow(640, 480, "Basic Graphics Starter", NULL, NULL);
    if (!_window) {
        glfwTerminate();
        exit(EXIT_FAILURE);
    }
    ...
    glfwSetKeyCallback(_window, App::key_callback);
    glfwSetWindowSizeCallback(_window, App::window_size_callback);
    glfwSetWindowPosCallback(_window, App::window_pos_callback);
    glfwSetCursorEnterCallback(_window, App::cursor_enter_callback);
    glfwSetCursorPosCallback(_window, App::cursor_position_callback);
    glfwSetMouseButtonCallback(_window, App::mouse_button_callback);
    glfwSetScrollCallback(_window, App::scroll_callback);
    ...
    // Example of how to load a model file.
    _model.reset(new Model("sphere.nff", 1.0, glm::vec4(0,0,1.0,1.0)));
...
void App::draw(const int windowWidth, const int windowHeight)
{
    ...
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (GLfloat)windowWidth / (GLfloat)windowHeight, 0.01f, 100.0f);
...
{
    std::cout<< "Key " << getKeyName(key)<< " "<<getActionName(action)<< std::endl;
}
...
{
    std::cout<< "Mouse button " << getButtonName(button) << getActionName(action) <<std::endl;
}
...
string App::getKeyValue(int key, int mods)
{
    string value = "";
    switch (key)
    {
...
string App::getActionName(int action)
{
    switch (action)
    {
        case GLFW_PRESS:
...
string App::getButtonName(int button)
{
    switch (button)
    {
...
string App::getModsName(int mods)
{
    string modName = "";
    if (mods & GLFW_MOD_SHIFT)
        modName += "SHIFT";
